...
comments and closes it. A step that recorded no verdict is itself red. Drills
and branch dispatches open/comment but never close; `dry_run` and
`ssh_targets_override` runs write nothing.
...
Feishu is paged and the issue is opened; the next scheduled run closes it.
