...
When `.github/workflows/out-of-band-watchdog.yml` cannot deliver to Feishu, it records
...
Per-check structured logs include `attempt_count` and `duration_ms` so transient
retries and slow paths are auditable in CI logs.
Run-level structured events include `run_duration_ms` for end-to-end execution
latency tracking.

Weekly summary is generated by `.github/workflows/watchdog-weekly-digest.yml`
(Monday UTC) using the out-of-band watchdog workflow run history and delivered via the same
Feishu mode. Digest output includes both success rate and failure rate, plus
recent failed run links. Use `workflow_dispatch` + `dry_run=true` for safe preview.
