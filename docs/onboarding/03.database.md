# 使用数据库

> **场景**：应用需要连接数据库（PostgreSQL、Redis、ClickHouse、ArangoDB）
> **预计时间**：30 分钟
> **前置条件**：已完成[部署第一个应用](./02.first-app.md)

---

## 概述

平台提供的数据库服务：

| 数据库 | 用途 | 访问方式 |
|--------|------|---------|
| **PostgreSQL** | 关系数据库（业务数据） | Vault 静态密码 |
| **Redis** | 缓存 + 消息队列 | Vault 静态密码 |
| **ClickHouse** | OLAP 分析 | Vault 静态密码 |
| **ArangoDB** | 图数据库 | Vault 静态密码 |

**核心机制**：所有数据库密码存储在 **Vault**，通过 **Kubernetes ServiceAccount + Vault Agent** 自动注入到应用 Pod。

---

## 快速选择场景

根据你的需求，直接跳到对应场景：

| 应用类型 | 推荐数据库 | 跳转 |
|---------|-----------|------|
| **Web 应用** | PostgreSQL | [场景 A](#场景-a-web-应用--postgresql) |
| **需要缓存的 Web 应用** | PostgreSQL + Redis | [场景 B](#场景-b-web-应用--缓存) |
| **数据分析应用** | PostgreSQL + ClickHouse | [场景 C](#场景-c-数据分析应用) |
| **图数据应用** | ArangoDB | [场景 D](#场景-d-图数据应用) |

---

## 数据库连接信息

### PostgreSQL (Business)

| 属性 | 值 |
|------|-----|
| **服务地址** | `postgresql.data-<env>.svc.cluster.local:5432` |
| **数据库名** | `business` |
| **Vault 路径** | `secret/data/postgres` |

**示例连接字符串**：
```
postgresql://user:password@postgresql.data-staging.svc.cluster.local:5432/business
```

### Redis

| 属性 | 值 |
|------|-----|
| **服务地址** | `redis-master.data-<env>.svc.cluster.local:6379` |
| **Vault 路径** | `secret/data/redis` |

### ClickHouse

| 属性 | 值 |
|------|-----|
| **HTTP 端口** | `8123` |
| **Native 端口** | `9000` |
| **服务地址** | `clickhouse.data-<env>.svc.cluster.local` |
| **Vault 路径** | `secret/data/clickhouse` |

### ArangoDB

| 属性 | 值 |
|------|-----|
| **服务地址** | `arangodb.data-<env>.svc.cluster.local:8529` |
| **Vault 路径** | `secret/data/arangodb` |

**注意**：`<env>` 为 `staging` 或 `prod`，根据你的部署环境替换。

---

## 场景 A: Web 应用 + PostgreSQL {#场景-a-web-应用--postgresql}

### A1. 联系运维创建 Vault Role {#a1-联系运维创建-vault-role}

**你需要提供**：
- 应用名称：`my-app`
- ServiceAccount 名称：`my-app`
- Namespace：`apps-staging`
- 需要访问的密钥：`secret/data/postgres`

**运维会做**：
在 `2.platform/` 创建类似如下的 Terraform 配置：

```hcl
# 2.platform/92.vault-myapp.tf

resource "vault_policy" "myapp" {
  name = "myapp"
  policy = <<-EOT
    path "secret/data/postgres" {
      capabilities = ["read"]
    }
  EOT
}

resource "vault_kubernetes_auth_backend_role" "myapp" {
  backend                          = vault_auth_backend.kubernetes.path
  role_name                        = "myapp"
  bound_service_account_names      = ["my-app"]
  bound_service_account_namespaces = ["apps-staging", "apps-prod"]
  token_policies                   = ["myapp"]
  token_ttl                        = 3600
}
```

### A2. 在 Kubero 配置 Vault Annotations

在你的 Deployment YAML 中（或通过 Kubero UI 配置）添加 annotations：

```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: my-app
spec:
  template:
    metadata:
      annotations:
        vault.hashicorp.com/agent-inject: "true"
        vault.hashicorp.com/role: "myapp"
        vault.hashicorp.com/agent-inject-secret-pg: "secret/data/postgres"
        vault.hashicorp.com/agent-inject-template-pg: |
          {{- with secret "secret/data/postgres" -}}
          export PGHOST="{{ .Data.data.host }}"
          export PGPORT="{{ .Data.data.port }}"
          export PGUSER="{{ .Data.data.username }}"
          export PGPASSWORD="{{ .Data.data.password }}"
          export PGDATABASE="{{ .Data.data.database }}"
          {{- end }}
    spec:
      serviceAccountName: my-app
      containers:
      - name: my-app
        image: your-image
        command: ["/bin/sh", "-c"]
        args:
          - |
            source /vault/secrets/pg
            exec node server.js
```

**关键点**：
1. `serviceAccountName` 必须匹配 Vault Role 配置
2. `vault.hashicorp.com/role` 指定 Vault role 名称
3. Template 将密钥渲染为环境变量导出语句
4. 启动脚本先 `source /vault/secrets/pg` 再启动应用

### A3. 应用代码读取环境变量

**Node.js 示例**：

```javascript
const { Pool } = require('pg');

const pool = new Pool({
  host: process.env.PGHOST,
  port: process.env.PGPORT,
  user: process.env.PGUSER,
  password: process.env.PGPASSWORD,
  database: process.env.PGDATABASE,
});

// 测试连接
pool.query('SELECT NOW()', (err, res) => {
  if (err) {
    console.error('Database connection failed:', err);
  } else {
    console.log('Database connected:', res.rows[0]);
  }
});
```

**Python 示例**：

```python
import os
import psycopg2

conn = psycopg2.connect(
    host=os.environ['PGHOST'],
    port=os.environ['PGPORT'],
    user=os.environ['PGUSER'],
    password=os.environ['PGPASSWORD'],
    database=os.environ['PGDATABASE']
)
```

### A4. 部署和验证

1. **部署应用**（Kubero 会自动重新部署）
2. **查看日志**：应该看到 "Database connected" 消息
3. **验证 Vault 注入**：
   ```bash
   kubectl exec -it <pod-name> -n apps-staging -- cat /vault/secrets/pg
   ```
   应该看到导出的环境变量。

---

## 场景 B: Web 应用 + 缓存 {#场景-b-web-应用--缓存}

需要同时使用 PostgreSQL 和 Redis。

### B1. 扩展 Vault Policy

```hcl
resource "vault_policy" "myapp" {
  name = "myapp"
  policy = <<-EOT
    path "secret/data/postgres" {
      capabilities = ["read"]
    }
    path "secret/data/redis" {
      capabilities = ["read"]
    }
  EOT
}
```

### B2. 添加 Redis annotations

```yaml
annotations:
  vault.hashicorp.com/agent-inject: "true"
  vault.hashicorp.com/role: "myapp"
  # PostgreSQL
  vault.hashicorp.com/agent-inject-secret-pg: "secret/data/postgres"
  vault.hashicorp.com/agent-inject-template-pg: |
    {{- with secret "secret/data/postgres" -}}
    export PGHOST="{{ .Data.data.host }}"
    export PGPORT="{{ .Data.data.port }}"
    export PGUSER="{{ .Data.data.username }}"
    export PGPASSWORD="{{ .Data.data.password }}"
    export PGDATABASE="{{ .Data.data.database }}"
    {{- end }}
  # Redis
  vault.hashicorp.com/agent-inject-secret-redis: "secret/data/redis"
  vault.hashicorp.com/agent-inject-template-redis: |
    {{- with secret "secret/data/redis" -}}
    export REDIS_HOST="{{ .Data.data.host }}"
    export REDIS_PORT="{{ .Data.data.port }}"
    export REDIS_PASSWORD="{{ .Data.data.password }}"
    {{- end }}
```

### B3. 应用代码

**Node.js Redis 连接**：

```javascript
const Redis = require('ioredis');

const redis = new Redis({
  host: process.env.REDIS_HOST,
  port: process.env.REDIS_PORT,
  password: process.env.REDIS_PASSWORD,
});

redis.on('connect', () => {
  console.log('Redis connected');
});
```

---

## 场景 C: 数据分析应用 {#场景-c-数据分析应用}

使用 PostgreSQL 存储数据 + ClickHouse 做 OLAP 分析。

参考场景 B 的模式，同时添加 ClickHouse secret：

```yaml
vault.hashicorp.com/agent-inject-secret-ch: "secret/data/clickhouse"
vault.hashicorp.com/agent-inject-template-ch: |
  {{- with secret "secret/data/clickhouse" -}}
  export CLICKHOUSE_HOST="{{ .Data.data.host }}"
  export CLICKHOUSE_PORT="{{ .Data.data.port }}"
  export CLICKHOUSE_PASSWORD="{{ .Data.data.password }}"
  {{- end }}
```

---

## 场景 D: 图数据应用 {#场景-d-图数据应用}

使用 ArangoDB。

```yaml
vault.hashicorp.com/agent-inject-secret-arango: "secret/data/arangodb"
vault.hashicorp.com/agent-inject-template-arango: |
  {{- with secret "secret/data/arangodb" -}}
  export ARANGO_HOST="{{ .Data.data.host }}"
  export ARANGO_PORT="{{ .Data.data.port }}"
  export ARANGO_PASSWORD="{{ .Data.data.password }}"
  {{- end }}
```

---

## 故障排查

### 问题 1: Pod 启动失败 - permission denied

**错误日志**：
```
Error: vault.hashicorp.com/agent-inject: permission denied
```

**原因**：Vault Role 未正确配置或 ServiceAccount 不匹配。

**排查**：
1. 确认 ServiceAccount 名称正确
2. 联系运维检查 Vault Role 配置：
   ```bash
   vault read auth/kubernetes/role/myapp
   ```

### 问题 2: Secret 文件为空

**排查**：
1. 查看 Vault Agent init 容器日志：
   ```bash
   kubectl logs <pod-name> -c vault-agent-init -n apps-staging
   ```
2. 确认 Vault secret 路径存在：
   ```bash
   vault kv get secret/data/postgres
   ```

### 问题 3: 应用连接数据库失败

**错误**：`connection refused` 或 `could not connect to server`

**排查**：
1. 确认数据库服务在对应 namespace 运行：
   ```bash
   kubectl get svc -n data-staging
   ```
2. 确认服务地址拼写正确（注意 `<env>` 替换）
3. 在 Pod 内测试连接：
   ```bash
   kubectl exec -it <pod-name> -n apps-staging -- \
     psql -h postgresql.data-staging.svc.cluster.local -U postgres
   ```

### 问题 4: Vault Agent 报错 - token expired

**原因**：Token TTL 过短或 Agent 未正确续期。

**解决**：联系运维调整 Vault Role 的 `token_ttl` 配置。

---

## 环境隔离注意事项

### Staging vs Prod 数据库完全独立

- `apps-staging` 使用 `data-staging` namespace 的数据库
- `apps-prod` 使用 `data-prod` namespace 的数据库
- **数据不会自动同步**

### 如何在环境间迁移数据？

**方式 1: 使用备份恢复**
```bash
# Staging 导出
kubectl exec -n data-staging postgresql-0 -- \
  pg_dump -U postgres business > staging_backup.sql

# Prod 导入
kubectl exec -i -n data-prod postgresql-0 -- \
  psql -U postgres business < staging_backup.sql
```

**方式 2: 使用数据同步工具**（推荐生产环境）
- 根据需要配置 ETL pipeline
- 联系运维团队协助

---

## 下一步

现在你的应用可以使用数据库了！🎉

根据你的需求，继续探索：

| 需求 | 下一步阅读 |
|------|----------|
| **管理其他密钥（API Keys 等）** | [管理密钥](./04.secrets.md) |
| **需要用户登录功能** | [接入 SSO](./05.sso.md) |
| **需要监控数据库查询** | [监控和分析](./06.observability.md) |

---

## 参考

- **数据库详细说明** - [db.overview.md](../ssot/db.overview.md)
- **Vault 接入机制** - [db.vault-integration.md](../ssot/db.vault-integration.md)
- **Vault 示例配置** - [db.vault-integration.md](../ssot/db.vault-integration.md)

---

*Last updated: 2025-12-21*
