...
| **Infra-012.14** | SigNoz and OpenPanel have automated synthetic write-then-query probes: SigNoz writes an OTLP log nonce through the collector and queries `signoz_logs.distributed_logs_v2`; OpenPanel writes a `/track` event nonce and queries `openpanel.events`. | **CI tests**: `libs/tests/test_observability_roundtrip_probe.py` validates both emit/query payloads and `libs/tests/test_probe_specs.py::test_synthetic_roundtrip_and_delivery_canaries_are_declared` verifies the probes are declared. |
| **Infra-012.15** | Alert delivery has a low-frequency real-send canary that posts a synthetic Alertmanager payload through `platform-alerting` to Feishu/Lark, with local state throttling so routine probes do not spam the channel. | **CI tests**: `libs/tests/test_alert_delivery_canary.py` validates payload, sending, and suppression; `libs/tests/test_probe_specs.py::test_synthetic_roundtrip_and_delivery_canaries_are_declared` verifies the probe is declared. |
