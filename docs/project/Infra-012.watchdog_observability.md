...
| **Infra-012.13** | Infra CI publishes a coverage context for `libs` and `tools` modules before introducing a fail-under threshold, so low-coverage operational paths are visible without blocking delivery prematurely. | **CI test**: `infra-ci.yml` runs `pytest` with `pytest-cov`, uploads `infra2-coverage-context`, and keeps `--cov-fail-under=0` until the baseline is reviewed. |
