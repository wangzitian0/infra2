...
- GitHub fallback now opens a `watchdog-alert-fallback` issue when Feishu
  delivery fails.
- Weekly digest workflow (`watchdog-weekly-digest.yml`) sends 7-day summary of
...
| **Infra-012.8** | Weekly digest job currently runs every Monday **01:00 UTC** via GitHub Actions (`watchdog-weekly-digest.yml`), aggregates the previous 7 days of out-of-band watchdog workflow runs (success/failure + failed run links), and sends a reliability summary to Feishu. **Future phase**: extend digest source to SigNoz probe-level aggregation by component + failure_domain. | **Metric**: By second week, report identifies 1-2 recurring failure patterns from watchdog run outcomes; SigNoz probe-level patterning is tracked as Phase-2 follow-up. |
...
| **Infra-012.13** | Infra CI publishes a coverage context for `libs` and `tools` modules before introducing a fail-under threshold, and the README exposes the main-branch Coveralls badge once main uploads begin. | **CI test**: `infra-ci.yml` runs `pytest` with `pytest-cov`, uploads `infra2-coverage-context`, uploads Cobertura XML to Coveralls on `main`, and keeps `--cov-fail-under=0` until the baseline is reviewed. |
