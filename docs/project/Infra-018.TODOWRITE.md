...
- [x] `finance_report/.github/workflows/`: replace direct `repo/tools/deploy_v2.py` execution with the versioned App deploy request.
- [x] `finance_report/tests/tooling/`: replace infra source-text assertions with SDK/wire-contract tests.
- [x] `finance_report/docs/`: replace `repo/docs` relative links with canonical infra URLs.
- [x] `infra2/repos/`: add SDK, Finance Report, and TrueAlpha as workspace-only submodules after both App edges are removed.
