...
| Infra-011.16 | Acceleration decisions are evidence-gated through the Env x Stage matrix: selected `iac_pinned` services may reconcile automatically from reviewed infra2 `main` only when fan-out evidence identifies changed inputs and the Deployer config-hash gate proves no-op vs restart; app production remains manual and environment-protected. | `docs/ssot/ops.pipeline.md`, `libs/tests/test_iac_runner_deploy_result.py`, `libs/tests/test_reconcile_iac_inputs.py`, `.github/workflows/reconcile-iac-inputs.yml` |
