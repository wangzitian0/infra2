...
| Infra-011.22 | Preview/canary acceptance uses only a deployment record created by the current trigger, fails closed when no record appears, ignores stale compose-level status, serializes the singleton canary slot, tests PR-head compose source, gives fresh-DB cold starts a bounded grace inside the outer readiness deadline, and reports cleanup success only after stable observed absence. | `libs/tests/test_preview_lifecycle.py`, `libs/tests/test_preview_teardown_convergence.py`, `libs/tests/test_deploy_v2_canary.py`, `libs/tests/test_sdk_contract_adoption.py`, `.github/workflows/ops-checks.yml`, `finance_report/finance_report/preview/compose.yaml`, `docs/ssot/ops.pipeline.md` |
...
| A compose exposes stale `composeStatus=error` before this trigger's deployment record exists. | Keep waiting for the new record; only an error on that current-operation record may fast-fail. |
