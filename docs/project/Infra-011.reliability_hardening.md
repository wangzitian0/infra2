...
| Infra-011.22 | A preview deploy reports success only after a deployment record created by its own trigger reaches a terminal-good state and every configured public surface proves the requested version. A stale response from the replaced stack, a terminal deployment error, a version mismatch, or a frontend that never starts must fail closed. Compose health budgets cover the measured migration-and-startup cold path. | `libs/tests/test_preview_lifecycle.py`, `libs/deploy/preview.py`, `finance_report/finance_report/preview/compose.yaml`, `docs/ssot/ops.pipeline.md` |
