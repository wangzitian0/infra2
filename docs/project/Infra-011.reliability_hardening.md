...
| Long-task time budget | `deploy-platform.yml`, IaC Runner sync, route canary, watchdogs, probe runner | Define soft and hard budgets, emit per-stage duration, and classify budget breaches separately from functional failures. | Making every deployment faster before measuring the slow stages. |
...
- Production deployment must remain manual and environment-protected.
...
| Infra-011.16 | Acceleration decisions are evidence-gated through the Env x Stage matrix: staging may skip or parallelize work only when stage evidence proves unchanged services and fallback coverage remains intact; production keeps full-sync/manual protection unless a separate AC proves equivalent safety. | `docs/ssot/ops.pipeline.md`, `libs/tests/test_iac_runner_deploy_result.py`, `.github/workflows/deploy-platform.yml` |
...
| IaC Runner `/health` returns degraded because a Python module or binary is missing. | `deploy-platform` fails during `iac-health-preflight`, not after starting `/deploy`. |
