...
| Infra-011.12 | CI/CD, watchdog, canary, and probe outputs separate environment from pipeline stage and share one sparse Env x Stage result schema with explicit stage names, failure domains, duration, deadline, external dependency flag, and suppression reason. The deploy_v2 Canary emits the released SDK shape on both health and alert paths. | `docs/ssot/ops.pipeline.md`, `docs/ssot/ops.alerting.md`, `libs/tests/test_pipeline_stage_contract.py`, `libs/tests/test_deploy_v2_canary.py` |
