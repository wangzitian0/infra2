...
| Infra-011.9 | (Superseded by #543) The hourly synthetic route canary is retired; public-route reachability derives from `PublicRouteFacet` probes, Dokploy control-plane status is consumed by the out-of-band watchdog's `run_dokploy_status_check` (fail-closed on missing API key), and deploy stalls page via the deploy-queue guard. | `libs/tests/test_out_of_band_watchdog.py`, `libs/tests/test_infra_probes.py` |
...
| Infra-011.12 | CI/CD, watchdog, canary, and probe outputs separate environment from pipeline stage and share a sparse Env x Stage result schema; deploy_v2 Canary is the first live producer. | `libs/tests/test_pipeline_stage_contract.py`, `libs/tests/test_deploy_v2_canary.py`, `docs/ssot/ops.pipeline.md`, `docs/ssot/ops.observability.md` |
| Infra-011.13 | External dependencies fail in preflight before expensive stages. | `libs/tests/test_pipeline_stage_contract.py`, `libs/tests/test_cloudflare_watchdog.py`, `libs/tests/test_out_of_band_watchdog.py` |
...
| Infra-011.22 | Preview/canary deploy proof is current-operation scoped, ambiguous create results reconcile without replaying non-idempotent writes, nested readiness budgets remain bounded, the singleton slot is serialized, and teardown proof requires stable observed absence and remains visible on failure. | `libs/tests/test_preview_lifecycle.py`, `libs/tests/test_preview_teardown_convergence.py`, `libs/tests/test_deploy_v2_canary.py`, `libs/tests/test_sdk_contract_adoption.py`, `.github/workflows/ops-checks.yml` |
...
- The first diagnostic raised only the Finance preview cold-start grace from 60 to 300 seconds under the existing 600-second public readiness deadline. PR canaries keep the exact head SHA as authority and clone the head branch only after proving it resolves to that SHA, so compose changes are tested without weakening ref red lines.
- Current-head live proof then exposed two tail cases: the cold start crossed 300 seconds under host load, and a non-idempotent `compose.create` committed despite its response timing out. The lifecycle now recovers the latter by deterministic-name read-after-write without replaying the POST, and the Finance inner grace is 450 seconds plus bounded retries (about 480 seconds total), retaining about 120 seconds for the unchanged outer public-route deadline.
- The first healthy current-head canary also exposed `status=pass` paired with `budget_status=hard-breach`: execution honored `--timeout 600`, while StageResult silently used the SDK's 180-second default. Evidence now derives `deadline_ms` from that same explicit timeout, leaving one hard-deadline source.
- Local proof: full `libs/tests` (`1346 passed`), focused ref/deploy_v2/rollout/preview/canary/SDK regressions (`204 passed`), CI-pinned Ruff, compileall, workflow YAML/contracts, MkDocs build, and `git diff --check` passed.
