...
- [ ] `cloudflare/infra-watchdog/worker.js`: 30 分钟 cron 完成后向独立 Healthchecks.io 检查报成功，异常发 `/fail`，停跑由 missed ping 告警；代码与行为测试已准备。部署 workflow 改为当前 head SHA 的 owner 手动 dispatch，避免 merge 自动触碰生产。待 Worker secret、生产部署与独立通知演练。
