...
- [ ] `tools/host_backup.sh` 现场覆盖验收：源代码已补齐 17/17 个 BackupFacet（原缺口另含 `platform/free`），并以双向 CI 检查防漂移；代码现把 prod/staging 的 manifest 与本地 `BACKUP_KEEP=7` 保留分别隔离，恢复演练默认只接受 Production manifest。2026-09-23 独立 Staging 本机 canary 产出 17/17，归档 282,318,058 字节，逐项 SHA256、大小、gzip/tar 可读性与权限检查通过；`BACKUP_REMOTE` 为空，不算异地证明。待 VPS 安装同 SHA 脚本并分别证明两环境的 17/17 异地 manifest、字节校验与新增类别隔离恢复。
