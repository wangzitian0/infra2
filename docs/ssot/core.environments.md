...
对 **preview** 多别名而言，`deployment.environment` 的取值就是别名 token 本身——`main` / `pr-<N>` / `commit-<sha7>`（见 §4.6）。同一份签发逻辑由 `libs/deploy/preview.py` 在 `up` 时通过 `ENV` 注入到 compose（vault-agent 与遥测共同消费），与 `tools/deploy_env_config.py::preview_alias` 派生的 `env_suffix` 保持同源。
...
带临时数据库的独立 compose 栈，由 `libs/deploy/preview.py` 手动 up/down。未来可在此
