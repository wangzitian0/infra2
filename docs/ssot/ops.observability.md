...
- **Cloudflare 预算(#904 验收)**:cron CPU p99 < 5 ms、每次运行子请求 ≤ 10、KV 只在状态变化时写(≤ 200/天),留在免费档。
  超出是设计缺陷,不是升级付费的理由。
- **可用率(#904 起为现状)**:成功计数与每 5 分钟的在场由 VPS 探针记录(重活,空槽即不可用);VPS 失联区间由 Worker 只在心跳过期/恢复两个边沿各写一次
...
| Cross-cutting | Docker container health | any container `unhealthy`/`starting`/`Restarting` outside a deploy window | P0/P1 | `container-breakdown-watch`(VPS);GitHub 日审计 `infra2-docker-health` 只报告(#908) |
...
- **在场(VPS)**:每轮还把所在的 5 分钟槽标记为在场(每环境每天 288 bit)。**没有任何一轮的槽就是不可用**——整机宕机、runner 崩溃、
  以及**部署重启**只要留下一个空槽,都照实计为不可用;这正是 Worker 边沿抓不到的那部分:短于心跳过期窗口(90 min)的中断。槽取 5 分钟
  而非 1 分钟,因为循环约 66 s 起一轮、全部超时的慢轮可达数分钟,1 分钟槽会把健康循环读成缺席;代价是短于一个槽的中断可能看不到。
...
- **合算**只在 `libs/availability_ledger.py`(纯函数,CLI 与测试共用):`apply_unavailability` 取空槽与失联区间的**并集**(重叠只计一次),
  按天、按探测周期(60 s)折算为该环境**每个**信号的失败次数——没有这一步,一台自己宕机的 VPS 会把幸存的日子报成 100%。读不懂的
  outage 记录计数并写进报告,不静默丢弃。
- **周报**:`tools/stability_report.py`(GitHub `ops-checks.yml` digest job,周一)经 SSH 读两个环境的账本文件(同
  `out_of_band_watchdog.py` 读备份 manifest 的方式),再取 Worker `/outages` → Lark。**production** 账本缺失、为空、超过 6 h 未更新或
  `/outages` 读不到,一律让任务失败并由该 job 的失败告警发出——不出报告好过出一份虚高的报告;**staging** 单独一行,缺失或过期只在报告里
  写明,不阻塞 production 报告。
- **账本文件的完整性**:类型不对的合法 JSON 与非法 JSON 一样视为损坏,移到 `*.corrupt.json` 后重新开始(否则每轮都会抛错、账本静默冻结);
  写入先 `fsync` 临时文件再原子 `rename`。
...
- **Page(直发 Feishu 告警群)**:`cloudflare-worker-status`(watchdog-liveness,**存活 = 在跑且能投递**:Worker 最近一次运行 ≤ 其 `WATCHDOG_STATUS_MAX_AGE_SECONDS`(值从 `cloudflare/infra-watchdog/wrangler.toml` 读,不另抄一份)且该次运行无 `deliveryError`;超龄、未来时钟偏差 >300s、从未运行、投递失败、`/status` 读不到 = 红)· `infra2-backup-production` 与 `infra2-restore-rehearsal`(data-protection)· `truealpha-scheduler-liveness`(peer-scheduler)· **watchdog 自身失效**:任何 `configuration` 失败(缺 SSH 配置、缺或被拒(401/403)的 `DOKPLOY_API_KEY`、缺 status token、registry 读不到)与报告通道缺失/投递失败。
- **哪些检查只报告由 [`watchdog-signals.yaml`](watchdog-signals.yaml) 决定**:`primary_owner: github` 且 `type: report` 的信号只报告(`dokploy-status:*` 各单元归入已注册的 `infra2-dokploy-status`);**其余一律 page**,包括只经 `vars.INFRA2_WATCHDOG_{HTTP,SSH}_TARGETS` 加入、registry 与审计都看不到的 target(未知 ≠ 只报告)。registry 读不到 → 全部 page(fail loud)。
- **只报告(日报,infra2 reports 群,`libs/alerting.py::deliver_infra2_report`,`INFRA2_REPORTS_FEISHU_{APP_ID,APP_SECRET,CHAT_ID}`)**:`infra2-public-entrypoint`、`infra2-ssh`、`infra2-docker`、`infra2-docker-health`(命令仍强制:任何 `unhealthy`/`starting`/`Restarting`/非零退出容器即红,不可移除)、`infra2-alert-bridge`、`infra2-backup-staging`(staging 永不 page)、Dokploy 状态族。它们照常运行、照常出结论,失败汇成**一条**日报;另附信息行:Worker 那次运行已自行投递的发现(`ok`/failures——Worker 已 page 过)与过期的 Dokploy 记录。**全绿的一天本 job 不发日报**:其它日报的送达已自证报告通道(§2)。
- **Dokploy 过期记录(#908)**:只凭**单元级**运行时证据判过期——经 SSH 按 compose project(`appName`)列出该单元自己的容器:全部 running 且 healthy(或无 healthcheck;one-shot 以 0 退出亦可),且带 `IAC_CONFIG_HASH` 的容器全部等于 Dokploy compose env 当前的 `IAC_CONFIG_HASH` → **过期记录**,列入日报供对账(ops.standards.md Rule 4),不算失败。全机 `infra2-docker-health` 绿**不算**证据(看不见拉镜像失败后旧版本仍健康运行);记录年龄也不算:超过 72h 且无单元证据 → 仍是(只报告的)失败,标注"old record"与年龄。无 SSH、列不出容器、输出读不懂 = 无证据。
- secrets:`INFRA2_WATCHDOG_SSH_{HOST,USER,PRIVATE_KEY}`、`INFRA2_WATCHDOG_WORKER_STATUS_TOKEN`、`DOKPLOY_API_KEY`、Feishu 告警投递 secrets + `INFRA2_REPORTS_FEISHU_*`(只注入 "Run watchdog" 这一步的 env,不进 job env)。退出码:有 page 为 1,仅报告为 0。结构化日志 `watchdog.check` 带 `route: page|report`(周 digest 只把 `page` 算作需召回的告警),过期记录为 `status: stale`。告警投递异常时发 `watchdog.delivery.failure` + 开 GitHub fallback issue(label `watchdog-alert-fallback`)+ 非零退出。
- **Issue 留痕(truealpha#876 W4)**:Feishu 之外(不替代),job 末步 `tools/watchdog_issue_trail.py` 为**每个红的 page 检查**维护一个 issue,标题**精确等于** `ops-checks watchdog is red: <check>`(label `incident`):红 → 已开则评论最早那条、否则新建;允许关闭的绿运行 → 评论并关闭同名 issue。**只报告的检查不记录结论、不开 issue(#908)**;在它们身上暴露的 watchdog 自身失效(及报告通道/registry 失效)记为 `out-of-band-watchdog` 这一步自己的红结论,下一次干净记录即关闭。**退役清扫**:各来源都已记录、且都声明记录完整(registry 读不到时 watchdog 声明不完整)时,允许关闭的运行把「无人再记录」的 trail issue(检查已改为只报告或已删除)评论并关闭;任一来源缺失或不完整则不清扫。各步把结论追加到 `INFRA2_WATCHDOG_VERDICTS_PATH`(`out-of-band-watchdog`、`iac-runner-health` 两个来源);**某来源没记录 = 该来源自身为红**(崩溃不等于安静)。列表失败**绝不**转为新建(警告,下次重试);列表成功后写失败 → 退出 1。issue 正文进公开仓库:去除 job 密钥型环境变量的值与 IP。模式(`libs/observability/issue_trail.py::issue_trail_mode` 与兼容垫片 `libs/watchdog_issue_trail.py`):schedule 或 main 上的普通 dispatch = 开/评/关;带 drill 上限或非 main 分支的 dispatch = 只开/评、永不关闭;`dry_run` 或 `ssh_targets_override` = 不写。权限:`issues: write` 只授予 watchdog job(digest job 为 `issues: read`),不在 workflow 级(deploy-v2-canary 会跑同仓 PR 代码)。
