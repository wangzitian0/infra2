...
- **Page(直发 Feishu 告警群)**:`cloudflare-worker-status`(watchdog-liveness,**只判新鲜度**:Worker 最近一次运行 ≤ 其 `WATCHDOG_STATUS_MAX_AGE_SECONDS`,值从 `cloudflare/infra-watchdog/wrangler.toml` 读,不另抄一份;超龄、未来时钟偏差 >300s、从未运行、`/status` 读不到 = 红)· `infra2-backup-production` 与 `infra2-restore-rehearsal`(data-protection)· `truealpha-scheduler-liveness`(peer-scheduler)· **watchdog 自身失效**:任何 `configuration` 失败(缺 SSH 配置、缺 `DOKPLOY_API_KEY`、缺 status token、registry 读不到)与报告通道缺失/投递失败。
- **哪些检查 page 由 [`watchdog-signals.yaml`](watchdog-signals.yaml) 决定**:`primary_owner: github` 且非 `type: report` 的信号 page,其余(含未注册的 `vars` 自定义 target、`dokploy-status:*`、`infra2-dokploy-status`)只报告。registry 读不到 → 全部 page(fail loud)。
- **只报告(日报,infra2 reports 群,`libs/alerting.py::deliver_infra2_report`,`INFRA2_REPORTS_FEISHU_{APP_ID,APP_SECRET,CHAT_ID}`)**:`infra2-public-entrypoint`、`infra2-ssh`、`infra2-docker`、`infra2-docker-health`(命令仍强制:任何 `unhealthy`/`starting`/`Restarting`/非零退出容器即红,不可移除)、`infra2-alert-bridge`、`infra2-backup-staging`(staging 永不 page)、Dokploy 状态族。它们照常运行、照常出结论,失败汇成**一条**日报;另附信息行:Worker 自己那次运行的结论(`ok`/failures/delivery error——Worker 自己 page 自己的发现,Healthchecks.io page 失败的运行)与过期的 Dokploy 记录。**全绿的一天本 job 不发日报**:其它日报的送达已自证报告通道(§2)。
- **Dokploy 状态时效(#908)**:`error` 单元的最新部署记录超过 **72h**,或同一运行里 `infra2-docker-health` 为绿(运行时证据与之矛盾)→ 记为**过期记录**,列入日报供对账(ops.standards.md Rule 4),不算失败;无时间戳且无健康运行时证据 → 仍是(只报告的)失败。
- secrets:`INFRA2_WATCHDOG_SSH_{HOST,USER,PRIVATE_KEY}`、`INFRA2_WATCHDOG_WORKER_STATUS_TOKEN`、`DOKPLOY_API_KEY`、Feishu 告警投递 secrets + `INFRA2_REPORTS_FEISHU_*`。退出码:有 page 为 1,仅报告为 0。结构化日志 `watchdog.check` 带 `route: page|report`(周 digest 只把 `page` 算作需召回的告警),过期记录为 `status: stale`。告警投递异常时发 `watchdog.delivery.failure` + 开 GitHub fallback issue(label `watchdog-alert-fallback`)+ 非零退出。
- **Issue 留痕(truealpha#876 W4)**:Feishu 之外(不替代),job 末步 `tools/watchdog_issue_trail.py` 为**每个红的 page 检查**维护一个 issue,标题**精确等于** `ops-checks watchdog is red: <check>`(label `incident`):红 → 已开则评论最早那条、否则新建;允许关闭的绿运行 → 评论并关闭同名 issue。**只报告的检查不记录结论、不开 issue(#908)**;在它们身上暴露的 watchdog 自身失效记为 `out-of-band-watchdog` 这一步自己的红结论,下一次干净记录即关闭。各步把结论追加到 `INFRA2_WATCHDOG_VERDICTS_PATH`(`out-of-band-watchdog`、`iac-runner-health` 两个来源);**某来源没记录 = 该来源自身为红**(崩溃不等于安静)。列表失败**绝不**转为新建(警告,下次重试);列表成功后写失败 → 退出 1。issue 正文进公开仓库:去除 job 密钥型环境变量的值与 IP。模式(`libs/observability/issue_trail.py::issue_trail_mode` 与兼容垫片 `libs/watchdog_issue_trail.py`):schedule 或 main 上的普通 dispatch = 开/评/关;带 drill 上限或非 main 分支的 dispatch = 只开/评、永不关闭;`dry_run` 或 `ssh_targets_override` = 不写。权限:`issues: write` 只授予 watchdog job(digest job 为 `issues: read`),不在 workflow 级(deploy-v2-canary 会跑同仓 PR 代码)。
...
| GitHub watchdog 只 page 本层故障类:page/报告分流、只报告检查不进 issue、Worker 只判新鲜度、Dokploy 72h 时效、报告通道缺失即 page(#908) | `libs/tests/test_out_of_band_watchdog.py` | ✅ |
