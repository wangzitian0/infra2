...
### 4.5.1 基础设施 synthetic round-trip probes

`platform/12.alerting` 的 `infra-probe-runner` 每 60 秒执行服务探针，但
synthetic 写读探针各自节流：

- `signoz-roundtrip` 默认每 5 分钟写入一条 OTLP log（service
  `infra-observability-canary`），再从
  `signoz_logs.distributed_logs_v2` 按 nonce 查回，证明 collector →
  ClickHouse 的 ingest/storage 链路可用。
- `openpanel-roundtrip` 默认每 5 分钟向 OpenPanel `/track` 写入
  `infra_observability_roundtrip_canary`，再从 `openpanel.events` 按 nonce
  查回，证明 API → worker/storage 链路可用。

节流窗口由 `OBS_ROUNDTRIP_INTERVAL_SECONDS` 控制；查询等待窗口由
`OBS_ROUNDTRIP_QUERY_WAIT_SECONDS` 控制。失败会作为
`InfraServiceProbeFailed` 进入 alert bridge，并通过 probe-runner heartbeat
暴露给 Cloudflare watchdog。
