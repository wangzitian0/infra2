...
| **备份归档** | **Cloudflare R2** | 异地冷备（off-host）。清单由各服务 `deploy.py` 的 `BackupFacet` 声明派生（`libs/backup_verification.py`，#542）；GitHub Actions 调度必须先登记到 [ops.pipeline.md](./ops.pipeline.md) 的 live workflow set，验证见 [`ops.recovery.md`](./ops.recovery.md) SOP-004/005 |
