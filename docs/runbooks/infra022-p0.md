...
**Brake / recovery:** The Worker deploy workflow is manual and requires the owner-approved exact `main` SHA. After approval, dispatch with `gh workflow run deploy-cloudflare-watchdog.yml --repo wangzitian0/infra2 --ref main -f approved_sha=<sha>`. Do not substitute an old SHA or suppress the external check to clear a page.
