...
- the app's variable schema or defaults — owned by the app (e.g. finance_report
  `config.py`), which **fails loud at boot** if a required deployed secret is
  missing.

The cross-repo secret contract (who declares, who injects, who supplies, and
when the app fails loud) is defined once, authoritatively, in
[finance_report `docs/ssot/deployment.md` → Secret Contract](https://github.com/wangzitian0/finance_report/blob/main/docs/ssot/deployment.md).
