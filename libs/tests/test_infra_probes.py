...
def test_the_full_dependency_route_is_probed_in_band_per_environment() -> None:
    ...
    assert (
        ...
        "https://report.zitian.party/api/health?full=1|200|warning|15||finance_report/app"
    ) in prod
    assert (
        "finance-report-api-full-health-public-route|http|"
        "https://report-staging.zitian.party/api/health?full=1|200|warning|15||finance_report/app"
    ) in staging
