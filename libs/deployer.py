...
def discover_services() -> dict[str, str]:
    """Auto-discover services by scanning for deploy.py files.

    Returns:
        dict mapping "layer/service" to "task_name.sync"
        Example: {"platform/postgres": "postgres.sync", "finance_report/app": "fr-app.sync"}
    """
    from pathlib import Path

    service_map = {}
    # Get repo root (parent of libs/)
    root = Path(__file__).parent.parent

    # Scan platform and finance_report layers
    for layer_dir in ["platform", "finance_report"]:
        ...
        for service_dir in layer_path.iterdir():
            ...
            # Extract service name from {nn}.{service} format
            ...
            if deploy_file.exists():
                # Build service key and task name
                if layer_dir == "finance_report":
                    # Special handling for nested finance_report structure
                    # finance_report/finance_report/{nn}.{service} -> finance_report/{service}
                    if layer_path.name == "finance_report":
                        service_key = f"finance_report/{service_name}"
                        task_name = f"fr-{service_name}.sync"
                    else:
                        service_key = f"{layer_dir}/{service_name}"
                        task_name = f"{service_name}.sync"
                else:
                    service_key = f"{layer_dir}/{service_name}"
                    task_name = f"{service_name}.sync"

                service_map[service_key] = task_name

    # Also scan bootstrap layer (though most won't have sync)
    bootstrap_path = root / "bootstrap"
    if bootstrap_path.exists():
        for service_dir in bootstrap_path.iterdir():
            if not service_dir.is_dir():
                continue
            ...
            if deploy_file.exists():
                service_key = f"bootstrap/{service_name}"
                # Use underscore for Vault compatibility (e.g., iac_runner)
                task_name = f"{service_name.replace('-', '_')}.sync"
                service_map[service_key] = task_name
