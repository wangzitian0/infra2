...
def bootstrap(c):
    ...
    if not fields:
        ...
        info("Create it with:")
        console.print(
            f'  op item create --category=login --title="{init_item}" --vault="{vault_name}" \\'
        )
...
def phase(c):
    ...
    phase_2 = bool(
        get_field("bootstrap/1password/VPS-01 Access Token: own_service", "credential")
    )
    ...
    table.add_row(
        "Phase 0: Local config",
        "[green]✅[/]" if phase_0 else "[dim]⏳[/]",
        f"{init_item}",
    )
    table.add_row(
        "Phase 2: 1Password Connect",
        "[green]✅[/]" if phase_2 else "[dim]⏳[/]",
        "Connect Token",
    )
    table.add_row(
        "Phase 3: Vault deployed",
        "[green]✅[/]" if phase_3 else "[dim]⏳[/]",
        "Root Token",
    )
