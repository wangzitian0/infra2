...
def ignored_paths(candidates: set[str]) -> set[str]:
    """Of ``candidates`` (repo-relative), those git is configured to ignore.

    A link can point at a file that is real in a working tree and real in CI
    and still be absent from the repository: generated documentation is built
    by a tool and gitignored rather than committed. ``os.path.exists`` then
    answers differently depending on whether the generator has run, so the
    check would pass locally and fail on a clean checkout -- the same
    depends-on-checkout-shape mistake as treating an unpopulated submodule as
    a dead link.

    Measured, not hypothesised: the sibling ``finance_report`` repository
    links at ``docs/reference/db-schema.md`` from 13 places. That file is
    generated by ``tools/generate_db_schema_reference.py``, gitignored, and
    rebuilt by its CI with a ``--check`` drift gate. Every one of those links
    is correct, and a checker without this concept calls all 13 dead.

    One ``git check-ignore --stdin`` for the whole candidate set, which is
    only ever the already-dead links. Exit 1 means "none ignored" and is not
    an error; anything else, including git being unavailable or this not
    being a repository, degrades to claiming nothing is ignored, so a broken
    git can only ever make this check stricter, never blind.
    """
    if not candidates:
        return set()
    try:
...
def main() -> int:
    ...
    for path in iter_markdown_files(SUBMODULES):
        ...
        for match in LINK_RE.finditer(text):
            ...
            if (rel_file, target) in KNOWN_UNRESOLVED:
                ...
                continue
            dead.append((rel_file, link, label, rel_target))

    # Generated-and-gitignored targets are real in a working tree and real in
    # CI, just never committed. Resolved after the walk so it costs one git
    # call over the dead set rather than one per link.
    generated_targets = ignored_paths({rel_target for *_, rel_target in dead})
    if generated_targets:
        built = [d for d in dead if d[3] in generated_targets]
        dead = [d for d in dead if d[3] not in generated_targets]
        print(
            f"{len(built)} link(s) point at generated, gitignored targets "
            "(present once their generator has run, absent from the repository "
            "by design):"
        )
        for rel_file, link, _label, _rel_target in built:
    ...
    for rel_file, link, label, _rel_target in dead:
        print(f"  {rel_file}")
        print(f"      [{label}]({link})")
    print(
