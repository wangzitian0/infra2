...
def install(c, version=DOKPLOY_VERSION):
    ...
    install_cmd = (
        f"curl -sSL https://dokploy.com/install.sh | DOKPLOY_VERSION={version} sh"
    )
...
def uninstall(c):
    ...
    result = run_with_status(
        c, f"ssh {vps_user}@{vps_host} '{uninstall_cmd}'", "Uninstalling Dokploy"
    )
...
def logs(c, lines="100"):
    ...
    c.run(f"ssh {vps_user}@{vps_host} 'docker logs --tail {lines} dokploy'", pty=True)
