...
def prepare(c):
    ...
    print("\n📁 Preparing Authentik data directories...")
    dirs = [
        "/data/bootstrap/authentik/postgres",
        "/data/bootstrap/authentik/redis",
        "/data/bootstrap/authentik/media",
        "/data/bootstrap/authentik/certs"
    ]
    for d in dirs:
        c.run(f"ssh root@{VPS_HOST} 'mkdir -p {d}'")
    
    # Set permissions (Authentik usually runs as 1000 or root, let's ensure 1000)
    c.run(f"ssh root@{VPS_HOST} 'chown -R 1000:1000 /data/bootstrap/authentik'")
    c.run(f"ssh root@{VPS_HOST} 'chmod -R 755 /data/bootstrap/authentik'")
    print("✅ Directories ready")

@task
def generate_env(c):
    """Generate local .env variables for Authentik (one-time)"""
    print("\n🔐 Generating Authentik secrets...")
    ...
    print("\n" + "="*60)
    print("📋 PLEASE ADD THESE TO YOUR DOKPLOY ENVIRONMENT VARIABLES:")
    print("="*60)
...
def deploy(c):
    ...
    print("   - Compose Path: bootstrap/06.authentik/compose.yaml")
    ...
    input("\n✋ Press Enter to continue after deployment completion...")

@task
def status(c):
    """Check Authentik status"""
    print(f"\n🔍 Checking Authentik status...")
